...
case class TweetEntity(id: BSONObjectID = BSONObjectID.generate, author: String, body: String)

object TweetEntity {

  implicit def toTweetEntity(tweet: Tweet) = TweetEntity(author = tweet.author, body = tweet.body)

  implicit object TweetEntityBSONReader extends BSONDocumentReader[TweetEntity] {

    def read(doc: BSONDocument): TweetEntity = TweetEntity(
    ...
    implicit object TweetEntityBSONWriter extends BSONDocumentWriter[TweetEntity] {
      def write(tweetEntity: TweetEntity): BSONDocument = BSONDocument(
        "_id" -> tweetEntity.id,
